...
def main():
    ...
    class MultiSigContract(sp.Contract):
        def __init__(self, members, requiredVotes):
            ...
            # Change 1 - Keep track of the proposal keys because the
            # Tezos Blockchain does not store keys in an enumerable format by design
            # This is done to optimize storage of large data on chain
            self.data.proposalKeys = sp.set()
        ...
        def submit_proposal(self, params):
            ...
            # change 2 - Add proposal's bigMapID to the proposalKeys array
            self.data.proposalKeys.add(self.data.activeProposalId)
